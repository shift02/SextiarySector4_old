...
public class MoistureStats {
    ...
    public void addStats(EntityPlayer entityPlayer, int par1, float par2) {
        ...
        // this.moistureSaturationLevel =
        // Math.min(Math.min(this.moistureSaturationLevel + par2,
        // this.moistureLevel), MAX_PREV_STAMINA_LEVEL);
        
        PlayerMoistureEvent.Add event = new PlayerMoistureEvent.Add(entityPlayer, moistureLevel,
                moistureSaturationLevel, par1, par2);
        
        if (MinecraftForge.EVENT_BUS.post(event)) {
            // this.moistureLevel = prevLevel;
            // this.moistureSaturationLevel = prevSaturation;
        } else {
            this.moistureLevel = MathHelper.clamp(event.newAddMoistureLevel + this.moistureLevel, 0,
                    MAX_MOISTURE_LEVEL);
            this.moistureSaturationLevel = MathHelper.clamp(
                    event.newAddMoistureSaturationLevel + this.moistureSaturationLevel, 0.0F, this.moistureLevel);
        }
    ...
    public void onUpdate(EntityPlayer par1EntityPlayer) {
        ...
        if (par1EntityPlayer.world.getGameRules().getBoolean("naturalRegeneration")
                && this.moistureLevel >= 18 && par1EntityPlayer.shouldHeal()) {
            ...
            if (this.moistureTimer >= 80) {
                if (par1EntityPlayer.getHealth() > 10.0F || i.getDifficultyId() >= 3
                    ...
                    // 1.0F);
                    SextiarySector4API.addStaminaExhaustion(par1EntityPlayer, 0.5f);
                }
    ...
    public void addExhaustion(EntityPlayer entityPlayer, float par1) {
        ...
        PlayerMoistureEvent.Exhaustion event = new PlayerMoistureEvent.Exhaustion(entityPlayer, moistureLevel,
                moistureExhaustionLevel, par1);
        
        if (MinecraftForge.EVENT_BUS.post(event)) {
            // this.moistureLevel = prevLevel;
            // this.moistureSaturationLevel = prevSaturation;
        } else {
            this.moistureExhaustionLevel = MathHelper
                    .clamp(event.newExhaustionLevel + this.moistureExhaustionLevel, 0, 40.0F);
        }
