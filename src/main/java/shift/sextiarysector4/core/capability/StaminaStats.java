...
import shift.sextiarysector4.api.event.player.PlayerMoistureEvent;
import shift.sextiarysector4.api.event.player.PlayerStaminaEvent;
...
public class StaminaStats {
    ...
    public void addStats(EntityPlayer entityPlayer, int par1, float par2) {
        ...
        // this.staminaLevel = Math.min(par1 + this.staminaLevel,
        // MAX_STAMINA_LEVEL);
        // this.staminaSaturationLevel =
        // Math.min(Math.min(this.staminaSaturationLevel + par1 * par2 * 2.0F,
        // this.staminaLevel), MAX_STAMINA_LEVEL / 2);
        
        PlayerMoistureEvent.Add event = new PlayerMoistureEvent.Add(entityPlayer, staminaLevel, staminaSaturationLevel,
                par1, par2);
        ...
        if (MinecraftForge.EVENT_BUS.post(event)) {
            ...
            this.staminaLevel = MathHelper.clamp(event.newAddMoistureLevel + this.staminaLevel, 0,
                    MAX_STAMINA_LEVEL);
            this.staminaSaturationLevel = MathHelper.clamp(
                    event.newAddMoistureSaturationLevel + this.staminaSaturationLevel, 0.0F, this.staminaLevel / 3.0f);
        }
     ...
     * this.addStats(par1ItemStaminaFood.getHealAmount(),
     * par1ItemStaminaFood.getSaturationModifier()); }
     */
    ...
    public void onUpdate(EntityPlayer par1EntityPlayer) {
        ...
        EnumDifficulty i = par1EntityPlayer.world.getDifficulty();
        ...
        if (par1EntityPlayer.world.getGameRules().getBoolean("naturalRegeneration")
                && this.staminaLevel >= 18 && par1EntityPlayer.shouldHeal()) {
            ++this.staminaTimer;
            ...
            if (this.staminaTimer >= 80) {
                if (par1EntityPlayer.getHealth() > 10.0F || i.getDifficultyId() >= 3
                        || par1EntityPlayer.getHealth() > 1.0F && i.getDifficultyId() >= 2) {
                    
                    if (!par1EntityPlayer.isPlayerSleeping()) {
                        // par1EntityPlayer.attackEntityFrom(DamageSource.cactus,
                        // 1.0F);
                        par1EntityPlayer.attackEntityFrom(DamageSource.STARVE, 1.0F);
                    }
                    // par1EntityPlayer.attackEntityFrom(DamageSource.starve,
                    // 1.0F);
                }
    ...
    public void addExhaustion(EntityPlayer entityPlayer, float par1) {
        ...
        PlayerStaminaEvent.Exhaustion event = new PlayerStaminaEvent.Exhaustion(entityPlayer, staminaLevel,
                staminaExhaustionLevel, par1);
        
        if (MinecraftForge.EVENT_BUS.post(event)) {
            // this.moistureLevel = prevLevel;
            // this.moistureSaturationLevel = prevSaturation;
        } else {
            this.staminaExhaustionLevel = MathHelper.clamp(event.newExhaustionLevel + this.staminaExhaustionLevel,
                    0, 400.0F);
        }
