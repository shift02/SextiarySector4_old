...
public class HUDEventHandler {
    ...
    public static final ResourceLocation icons = new ResourceLocation(SextiarySector4.MODID, "textures/guis/icons.png");
    public static final ResourceLocation progress = new ResourceLocation(SextiarySector4.MODID, "textures/guis/progress.png");
    ...
    public void onRenderGameOverlayEventPre(RenderGameOverlayEvent.Pre event) {
         ...
         * mc.playerController.shouldDrawHUD()){ renderMoisture( width, height);
         * renderStamina( width, height); //renderSeason( width, height); }
         */
        
        /*
         * if(event.type ==
         * ElementType.ARMOR&&ForgeHooks.getTotalArmorValue(mc.thePlayer)>0){
         * //System.out.println("AIR"); if(!ARMOR){
         * GuiIngameForge.left_height+=(left_height-29); ARMOR =false; }else{
         * GuiIngameForge.left_height-=(left_height-29); ARMOR =true; } }
         */
        
        if (event.getType() == RenderGameOverlayEvent.ElementType.ALL) {
            visibleStamina = GuiIngameForge.renderFood | GuiIngameForge.renderHealthMount | GuiIngameForge.renderHealth;
            visibleMoisture = GuiIngameForge.renderFood | GuiIngameForge.renderHealthMount
            ...
            /*
             * if (visibleMoisture) { renderMoisture(width, height);
             * visibleMoisture = false; } if (visibleStamina) {
             * renderStamina(width, height); visibleStamina = false; }
             */
        }
    ...
    public void onRenderGameOverlayEventPost(RenderGameOverlayEvent.Post event) {
        ...
        if (event.getType() == RenderGameOverlayEvent.ElementType.FOOD
            ...
            /*
             * if (!visibleMoisture && isPM) { HUDMP.renderMoney(width, height);
             * HUDMP.renderAddMoney(width, height); isPM = false;
             * HUDMP.isRenderer = false; }
             */
    ...
    // アイテムテキストずらし
    // @SubscribeEvent
    // public void onRenderGameOverlayEventPost(RenderGameOverlayEvent.Post
    // event) {
    
    /*
     * if(event.type == ElementType.EXPERIENCE||event.type ==
     * ElementType.JUMPBAR){ }
     */
    ...
    protected void renderMoisture(int width, int height) {
        ...
        MoistureStats stats = EntityPlayerManager.getMoistureStats(mc.player);
        int level = EntityPlayerManager.getPrevMoistureLevel(mc.player);
        // int levelLast = stats.getPrevFoodLevel();
        
        // System.out.println("BBB "+level+" : "+stats.getMoistureLevel()+" :
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            if (unused) backgound = 1; // Probably should be a += 1 but vanilla
                                       // never uses this
            ...
            /*
             * if (unused) { if (idx < levelLast) drawTexturedModalRect(x, y,
             * icon , iconY, 9, 9); else if (idx == levelLast)
    ...
    protected void renderStamina(int width, int height) {
        ...
        int level = EntityPlayerManager.instance.getStaminaLevel(mc.player);// getPrevStaminaLevel(mc.thePlayer);
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            /*
             * if (mc.thePlayer.isPotionActive(Potion.hunger)) { icon += 36;
             * backgound = 13; }
             */
            if (unused) backgound = 1; // Probably should be a += 1 but vanilla
                                       // never uses this
    ...
    protected void renderProgress(int width, int height) {
        ...
        // OpenGlHelper.glBlendFunc(GL11.GL_ONE_MINUS_DST_COLOR,
    ...
    public void drawTexturedModalRect(int x, int y, int textureX, int textureY, int width, int height) {
        ...
        vertexbuffer.pos(x + 0, y + height, zLevel)
                .tex((textureX + 0) * 0.00390625F, (textureY + height) * 0.00390625F).endVertex();
        vertexbuffer.pos(x + width, y + height, zLevel)
                .tex((textureX + width) * 0.00390625F, (textureY + height) * 0.00390625F).endVertex();
        vertexbuffer.pos(x + width, y + 0, zLevel)
                .tex((textureX + width) * 0.00390625F, (textureY + 0) * 0.00390625F).endVertex();
        vertexbuffer.pos(x + 0, y + 0, zLevel)
                .tex((textureX + 0) * 0.00390625F, (textureY + 0) * 0.00390625F).endVertex();
        tessellator.draw();
    }
